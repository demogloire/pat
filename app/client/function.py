...
def produit_panier():
    if current_user.type_user=='Client':
        return Panier.query.filter_by(user_id=current_user.id).all()
    else:
        return redirect(url_for('main.main'))


def autorisation_client(f):
    @wraps(f)
...
def numero_commande(current_user):
    numero_panier_id=Commande.query.order_by(Commande.id.desc()).first()
    id_client_commane=None
    if numero_panier_id is None:
        id_client_commane=f"Facture #{1}{current_user.id}"
        return id_client_commane
    else:
        id_encours=numero_panier_id.id + 1
        id_client_commane=f"Facture #{id_encours}{current_user.id}"
        return id_client_commane
